module ActivityLogger
  ...
  def add_activities(options = {})
    ...
    # todo - return if person and (!person.email_verified? or person.deactivated?)
    ...
    people_ids = people_to_add(person, artist, activity, include_person)
    do_feed_insert(people_ids, activity.id) unless people_ids.empty?
    do_artist_feed_insert(artist.id, activity.id) unless artist.nil?
  end
  ...
  def people_to_add(person, artist, activity, include_person=false)
    ...
    unless artist.nil?
      # assuming
      all + artist.members.map(&:id)
      all + artist.fans.map(&:id)
    end
