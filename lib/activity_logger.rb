module ActivityLogger
  ...
  def add_activities(options = {})
    entity = options[:person] || options[:artist]
    include_person = options[:include_person]

    # short circuit if this is an unverified user
    return if entity.is_a? Person and (!entity.email_verified? or entity.deactivated?)

    if options[:activity]
      activity = options[:activity]
    elsif entity.is_a? Person
      activity = Activity.create(:item => options[:item], :person => entity)
    else
      activity = Activity.create(:item => options[:item], :artist => entity)
    end
  ...
  def people_to_add(entity, activity, include_person=false)
    if entity.is_a? Person
      all = entity.contacts.map(&:id)
      all.push(entity.id) if include_person
    else
      # assuming
      all = entity.members.map(&:id)
      all + entity.fans.map(&:id)
    end
