module CustomModelMatchers
  ...
  class ContainMemberWithPerson
    ...
    def failure_message
      "expected #{@target.inspect} to contain person #{@expected}"
    end

    def negative_failure_message
      "expected #{@target.inspect} not contain #{@expected}"
    end
  end

  def contain_member_with_person(expected)
    ContainMemberWithPerson.new(expected)
  end
end